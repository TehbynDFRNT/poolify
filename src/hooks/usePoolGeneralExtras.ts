...
export const usePoolGeneralExtras = (poolId: string, customerId: string | null) => {
    ...
    }, [customerId, poolId, generalExtras, state, queryClient]);
    ...
    const addMiscItem = (extraId: string, quantity: number = 1) => {
        setState(prev => ({
            ...prev,
            miscItems: {
                items: [...prev.miscItems.items, { extraId, quantity }]
            }
        }));
    };
    ...
    const removeMiscItem = (extraId: string) => {
        setState(prev => ({
            ...prev,
            miscItems: {
                items: prev.miscItems.items.filter(item => item.extraId !== extraId)
            }
        }));
    };
    ...
    const updateMiscItemQuantity = (extraId: string, quantity: number) => {
        setState(prev => ({
            ...prev,
            miscItems: {
                items: prev.miscItems.items.map(item =>
                    item.extraId === extraId ? { ...item, quantity } : item
                )
            }
        }));
    };

    // Get a specific general extra by ID
    const getExtraById = (id: string): GeneralExtra | undefined => {
        return generalExtras?.find(extra => extra.id === id);
    };
