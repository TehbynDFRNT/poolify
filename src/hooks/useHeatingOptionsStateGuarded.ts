...
interface UseHeatingOptionsStateGuardedProps {
    poolId: string | null;
...
export function useHeatingOptionsStateGuarded({
    poolId,
    customerId,
    compatibleHeatPump,
    blanketRoller,
    getInstallationCost
}: UseHeatingOptionsStateGuardedProps) {
    ...
    const {
        ...
        mutationFn: async () => {
            if (!customerId || !poolId) {
                throw new Error("Customer and pool information is required to save heating options");
            }
