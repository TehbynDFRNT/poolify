...
export const TotalCostSummary: React.FC = () => {
  ...
  // Calculate totals
  const extraPavingCost = quoteData.extra_paving_cost || 0;
  ...
  const shouldDisplaySection = (value: number): boolean => {
    return value > 0;
  };

  // Determine if main paving cost should be displayed (excluding existing concrete paving)
  const mainPavingCost = extraPavingCost - existingConcretePavingCost;
  const hasMainPavingCost = mainPavingCost > 0;
