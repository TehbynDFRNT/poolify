...
import { GeneralExtraType } from "@/types/general-extra";
import { useState } from "react";
import { toast } from "sonner";

interface AddGeneralExtraFormProps {
    open: boolean;
    onOpenChange: (open: boolean) => void;
}

export function AddGeneralExtraForm({ open, onOpenChange }: AddGeneralExtraFormProps) {
    const { addGeneralExtra } = useGeneralExtras();
    const [isSubmitting, setIsSubmitting] = useState(false);
    const [formData, setFormData] = useState({
        name: "",
        sku: "",
        type: "Misc" as GeneralExtraType,
        description: "",
        cost: "",
        margin: "",
        rrp: "",
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        try {
            ...
            console.error("Error adding general extra:", error);
            toast.error("Failed to add general extra");
        } finally {
            setIsSubmitting(false);
        }
    };
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                    <DialogTitle>Add New General Extra</DialogTitle>
                </DialogHeader>
