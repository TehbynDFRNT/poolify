...
export const useWaterFeatureGuarded = (customerId?: string | null, poolId?: string | null) => {
    ...
    const {
        mutate: saveWaterFeatureMutation,
        isPending: isSubmitting,
        StatusWarningDialog: SaveStatusWarningDialog
    } = useGuardedMutation({
        projectId: customerId || '',
        mutationFn: async (values: WaterFeatureFormValues) => {
            if (!customerId || !poolId) {
                throw new Error("Missing customer or pool information");
            }
    ...
    const {
        mutate: deleteWaterFeatureMutation,
        ...
        mutationFn: async () => {
            if (!customerId || !poolId || !existingData) {
                throw new Error("No water feature options to remove");
            }
