...
export const RetainingWallCostSummary: React.FC<RetainingWallCostSummaryProps> = ({ 
  ...
  // Fetch retaining wall data when component mounts
  useEffect(() => {
    if (customerId) {
      fetchWallData();
    }
  ...
  const fetchWallData = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        setWalls(wallSummaries);
        setTotalMargin(parseFloat(totalMarginAmount.toFixed(2)));
        setTotalCost(parseFloat(totalCostAmount.toFixed(2)));
      }
    } catch (error) {
