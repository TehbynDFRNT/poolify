...
export const TrueCostCell = ({ poolId, poolCost, packageInfo, pool }: TrueCostCellProps) => {
  ...
  const getTrueCost = () => {
    const isEmpire = pool.name.includes('Empire');
    const logPrefix = isEmpire ? '🔍 EMPIRE POOL - ' : '';
    ...
    console.log(`${logPrefix}Column 15 - Pool buy price (inc GST):`, poolBuyPrice);
    
    // Column 17: Filtration Package Price
    const filtrationCost = packageInfo?.price || 0;
    console.log(`${logPrefix}Column 17 - Filtration package cost:`, filtrationCost);
    
    // Column 19: Crane Cost
    const craneCost = getCraneCost(poolId);
    console.log(`${logPrefix}Column 19 - Crane cost:`, craneCost);
    
    // Column 21: Excavation Cost
    const excavationCost = calculateExcavationCost(poolId);
    console.log(`${logPrefix}Column 21 - Excavation cost:`, excavationCost);
    ...
    console.log(`${logPrefix}Column 29 - Construction costs:`, {
      ...constructionCosts,
    ...
    console.log(`${logPrefix}Column 40 - Fixed costs total:`, fixedCostsTotal);
    ...
    console.log(`${logPrefix}TOTAL TRUE COST (sum of columns 15+17+19+21+29+40):`, trueCost);
    
    if (isEmpire) {
      console.log(`🔍 EMPIRE POOL - BREAKDOWN SUMMARY:`);
      console.log(`  Column 15 (Buy Price inc GST):    ${formatCurrency(poolBuyPrice)}`);
      console.log(`  Column 17 (Filtration Package):   ${formatCurrency(filtrationCost)}`);
      console.log(`  Column 19 (Crane Cost):           ${formatCurrency(craneCost)}`);
      console.log(`  Column 21 (Excavation Cost):      ${formatCurrency(excavationCost)}`);
      console.log(`  Column 29 (Construction Costs):   ${formatCurrency(constructionCostsTotal)}`);
      console.log(`  Column 40 (Fixed Costs):          ${formatCurrency(fixedCostsTotal)}`);
      console.log(`  ------------------------------------------------`);
      console.log(`  TOTAL TRUE COST:                  ${formatCurrency(trueCost)}`);
    }
